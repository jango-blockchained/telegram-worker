...
async function handleProcessRequest(request, env) {
	...
	try {
		...
		if (!expectedInternalKey) {
			console.error("INTERNAL_KEY_BINDING secret not configured or accessible.");
			return new Response(
		...
		if (!message) {
			return new Response(
				JSON.stringify({ success: false, error: "Missing message in payload", result: null }),
				{ status: 400, headers: { "Content-Type": "application/json" } }
			);
		}
		...
		if (!chatId) {
			...
			return new Response(
				JSON.stringify({ success: false, error: "Chat ID configuration error", result: null }),
				{ status: 500, headers: { "Content-Type": "application/json" } }
			);
		}

		// --- Send Telegram Message --- 
		console.log(`Sending message for request ${incomingRequestId} to chat ID ${chatId}`);
		...
		console.error(`Error processing Telegram request ${incomingRequestId}:`, error);
		return new Response(
			JSON.stringify({
				success: false,
				error: error.message || "Unknown error occurred sending Telegram message",
				result: null,
			}),
			{ status: 500, headers: { "Content-Type": "application/json" } }
		);
