...
describe("Telegram Worker", () => {
    ...
    const createMockEnv = (secrets: { internalKey?: string | null, botToken?: string | null, chatId?: string | null }) => ({
    ...
    test("rejects request with invalid internal key", async () => {
        mockEnv = createMockEnv({ botToken: TEST_BOT_TOKEN, chatId: TEST_CHAT_ID, internalKey: null }); // No internal key configured
    ...
    test("sends telegram message with explicit chat ID", async () => {
        const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-1" },
            body: JSON.stringify(validNotification)
        });
    ...
    test("sends telegram message with default chat ID from binding", async () => {
        const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-2" },
    ...
    test("returns error if default chat ID binding fails and no chat ID in request", async () => {
        mockEnv = createMockEnv({ internalKey: TEST_INTERNAL_KEY, botToken: TEST_BOT_TOKEN, chatId: null }); // Default chat ID is null
        const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-3" },
    ...
    test("returns error if bot token binding fails", async () => {
        mockEnv = createMockEnv({ internalKey: TEST_INTERNAL_KEY, botToken: null, chatId: TEST_CHAT_ID }); // Bot token is null
        const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-4" },
            body: JSON.stringify(validNotification)
        });
    ...
    test("handles Telegram API fetch errors", async () => {
        ...
        const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-5" },
            body: JSON.stringify(validNotification)
        });
    ...
    test("handles non-200 response from Telegram API", async () => {
         ...
         const request = new Request("https://telegram-worker.workers.dev", {
            method: "POST",
            headers: { /* ... headers ... */ "X-Internal-Key": TEST_INTERNAL_KEY, "X-Request-ID": "req-6" },
            body: JSON.stringify(validNotification)
         });
